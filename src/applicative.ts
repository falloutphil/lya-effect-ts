...
// Create a version of ap specifically for our Applicative instance
const ap = SA.ap(O.SemiApplicative);

// Using SemiApplicative's `ap` method to apply the curried functions
const result = pipe(
  addWrapped,                                    // Start with the wrapped function
  ap(Option.some(3)),  // Apply to the first Option (x = 3)
  ap(Option.some(5))   // Apply to the second Option (y = 5)
);
...
const result2 = pipe(
  ...
  ap(Option.some(10)), // Apply to the first Option (x = 10)
  ap(Option.some(6)),  // Apply to the second Option (y = 6)
  ap(Option.some(3))   // Apply to the third Option (z = 3)
);
